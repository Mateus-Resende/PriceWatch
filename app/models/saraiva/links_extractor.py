...
class LinksExtractor(scrapy.Spider):
  ...
  name = 'spider'
  start_urls = ['http://busca.saraiva.com.br/pages/informatica/notebook']
  download_delay = 1.5

  def parse(self, response):
    for vitrine in response.css('.cs-product-title'):
      link_data = { 
    ...
    link_next = response.css('.next::attr("href")').extract_first()
    if link_next:
