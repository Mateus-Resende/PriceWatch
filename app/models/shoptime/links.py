...
class SpiderShoptime(scrapy.Spider):

  name = 'spider'
  start_urls = ['http://www.shoptime.com.br/linha/320352/informatica/notebook']
  download_delay = 1.5

  def parse(self, response):
    for vitrine in response.css('.p-card-wrapper'):
      link_data = { 
    ...
    link_next = response.xpath("//*[@class='smooth-scroll pure-button  next']/@href").extract_first()
    if link_next:
